...
**Why deduplication merges, rather than just flagging:** when a returning prospect submits a second form, there are two ways to handle the collision. The simpler one is to flag the new record as a possible duplicate and let a person sort it out later. The other is to merge it into the existing record automatically, at the moment of capture. Merging was chosen because flagging just defers the problem to a moment when it is harder to solve: by the time someone reviews the flag, automated messages may already have gone out against the duplicate, and the lead's history is split across two records. Merging at the first step keeps one person as one record with one continuous history, which every later step depends on. The trade-off is that an automatic merge must be conservative — it matches only on a strong identifier, phone or email, never on a fuzzy match like a similar name, because a wrong merge fuses two different people and that is worse than a duplicate.
...
**Why a fixed sequence and not event-based follow-up:** there were two defensible options here. One is event-based follow-up, where the timing of the next message reacts to signals — the lead opened the last message, visited the site, and so on. It is more responsive and, done well, can feel more personal. The other is a fixed, pre-decided cadence that runs the same way every time. The fixed sequence was chosen deliberately. Event-based timing adds a layer of logic that can fail quietly and is hard for a non-technical team member to reason about: when a lead does *not* get a message, nobody can easily say why. A fixed cadence is fully predictable — anyone can look at a lead and know exactly what it has received and what is coming next. For a small team that has to trust and operate this system, predictability is worth more than the extra responsiveness. The cadence itself is the balance: spaced far enough apart not to look desperate, close enough not to go cold.
...
**Why "no response" leads go to Nurture and are not simply closed:** the tempting alternative is to mark an unresponsive lead as dead and remove it. It keeps the pipeline clean and the numbers honest. But it throws away two things: the marketing spend that produced the lead, and the fact that silence is not rejection — a lead who did not reply this month may just have been busy. Keeping every unresponsive lead in active follow-up forever is the opposite error: it clogs the pipeline and risks annoying people into opting out. Nurture is the deliberate middle path. The lead leaves the active sequence, so the pipeline stays clean and honest, but it is not destroyed — it moves to low-frequency, long-term contact. The door stays open without the contact being burned, and the acquisition cost is not written off after six days. The one case where a lead *is* fully closed is invalid contact data, because there the issue is not silence, it is that the lead was never reachable at all.
