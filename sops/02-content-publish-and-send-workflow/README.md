...
**Why it is generated, not written again:** rewriting the content into an email by hand is duplicated effort and a fresh chance to introduce errors. Deriving the newsletter from the published piece means it is consistent with the website by construction. If the content changes, regenerating the newsletter is one action, not a manual re-do.

**Why it stops at a draft:** this is the deliberate pause in the procedure. The system has done all the repetitive work, but it has not sent anything. Generation and sending are kept separate on purpose, so that a human always sees the finished newsletter before the audience does.
...
**Why a human review step is mandatory and not skippable:** a newsletter goes to the whole audience at once and cannot be recalled. The cost of a mistake is asymmetric — an unreviewed automated send that goes out wrong damages trust with every subscriber simultaneously. The review step is cheap; a bad send is not. This is the one place where the procedure deliberately slows down.
...
- **Content is edited after publishing** — the published piece remains the source of truth, so the newsletter is regenerated from the updated content. A newsletter that has not been sent yet is simply replaced. A newsletter that has already been sent is not recalled; if the change is significant, that is a judgment call for the content owner, not something the system decides.
